        ...
        if not cont:
            ...
            if sents[1] in lib:
                for j in range(len(lib[sents[1]])):
                    if (
                        lib[sents[1]][j]["回答"] == sents[2 + i]
                        and lib[sents[1]][j]["伺服器"] == message.guild.name
                    ):
...
async def forget(message, sents):
    if (
        message.author.id in datas["master_id"]
        or datas["can_forget"]
        and len(sents) > 1
    ):
                    ...
                    for j in range(len(lib[sents[1]])):
                        if lib[sents[1]][j]["回答"] == sents[2 + i]:
                            ...
                            await message.channel.send(
                                datas["forget"].format(sents[2 + i])
                            )
                            cont = False
                            break
                    if cont and not sents[1] in findlib.keys():
                        await message.channel.send(
                            datas["do_not_know"].format(sents[2 + i])
                        )
                with open("datas/sentence_library.json", "w", encoding="utf-8") as f:
                    json.dump(lib, f, ensure_ascii=False)
        ...
        for i in range(len(sents) - 1):
            ...
            for j in range(len(ban_words[message.guild.name])):
                if ban_words[message.guild.name][j] == sents[i + 1]:
                    ...
                    await message.channel.send(
                        datas["lift_ban_word"].format(sents[i + 1])
                    )
    ...
    for i in range(len(sents) - 2):
        ...
        if not cont:
            ...
            if sents[1] in findlib.keys():
                for j in range(len(findlib[sents[1]])):
                    if (
                        findlib[sents[1]][j]["回答"] == sents[2 + i]
                        and findlib[sents[1]][j]["伺服器"] == message.guild.name
                    ):
                        await message.channel.send(datas["have_learnt"])
                        cont = False
                        break
...
async def draw(message, sents):
    await message.channel.send(
        datas["draw_word"].format(random.choice(sents[1 : len(sents) - 1]))
    )
...
async def help(message, sents):
    await message.channel.send(
        ...
        """.format(
            datas["learn_command"],
            datas["forget_command"],
            datas["ban_command"],
            datas["lift_ban_command"],
            datas["find_command"],
            datas["draw_command"],
            datas["trans_to_ch"],
            datas["trans_to_ja"],
...
async def set_administrator(message, sents):
    ...
    if not message.author.id in datas["master_id"]:
        ...
        await message.channel.send(
            datas["set_administrator"].format(message.author.name)
        )
...
async def change_command(message, sents):
    ...
    del funcs[sents[1]]
    await message.channel.send(
        datas["change_command_word"].format(message.author.name, sents[1], sents[2])
    )
    for channel in datas["system_channels"]:
        await client.get_channel(channel).send(
            datas["change_command_word"].format(message.author.name, sents[1], sents[2])
...
async def change_add_ban_permission(message, sents):
    ...
    for channel in datas["system_channels"]:
        await client.get_channel(channel).send(
            datas["change_permission_word"].format(
                message.author.name,
                "can_add_ban",
                not datas["can_add_ban"],
                datas["can_add_ban"],
            )
        )
    with open("datas/datas.json", "w", encoding="utf-8") as f:
    ...
    for channel in datas["system_channels"]:
        await client.get_channel(channel).send(
            datas["change_permission_word"].format(
                message.author.name,
                "sentence_common",
                not datas["sentence_common"],
                datas["sentence_common"],
            )
        )
    with open("datas/datas.json", "w", encoding="utf-8") as f:
        json.dump(datas, f, ensure_ascii=False)
